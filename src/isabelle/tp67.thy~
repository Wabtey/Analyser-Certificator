...
definition "(statST1::staticSymTable)= [(''x'',[Some 5, Some 0]),(''y'',[None, Some 2, Some 17])]"
...
(*
  placeholder x ys adds x to each ys' element
  DOC: rename
 *)
fun placeholder::"int \<Rightarrow> int list \<Rightarrow> int list"
  where
"placeholder _ [] = []" |
"placeholder x (y # ys) = ((x + y) # (placeholder x ys))"

value "placeholder 0 [0,1,-1]"
value "placeholder 1 [0,1,-1]"
...
fun mapAddition::"int list \<Rightarrow> int list \<Rightarrow> int list"
  where
"mapAddition [] res = res" |
"mapAddition (x # xs) ys = mapAddition xs (placeholder x ys)"

(* TODO: mapSubtraction *)
fun mapSubtraction::"int list \<Rightarrow> int list \<Rightarrow> int list"
  where
"mapSubtraction xs ys = mapAddition xs ys"
...
"san (Seq s1 s2) symt = (
  let (s1ST, s1IsOK) = san s1 symt in
    let (s2ST_, s2IsOK) = san s2 s1ST in
      (s2ST, s1IsOK \<and> s2IsOK)
)" |
(* draw all possible staticSymTable for each arm and return the concat of the two *)
"san (If c s1 s2) symt = (san s1 symt \<and> san s2 symt)" |
...
lemma correctionSan: "san s \<longrightarrow> \<not>BAD (evalS s ([],i,[]))"
  nitpick
  quickcheck
  apply auto
  sledgehammer
